...
namespace Reth.Itss2.Dialogs.Standard.UnitTests.Serialization.Xml.Tokenization
{
    ...
    public class TokenizerTest
    {
        ...
        {
            using( Tokenizer tokenizer = new Tokenizer( this.BaseStream ) )
            {
                ...
                observerMock.Setup( x => x.OnNext( It.IsAny<ReadOnlySequence<byte>>() ) ).Callback(     ( ReadOnlySequence<byte> token ) =>
                                                                                                        {
                                                                                                            actualMessages.Add( XmlSerializationSettings.Encoding.GetString( token.ToArray() ) );
                                                                                                        }   );
        ...
        {
            using( Tokenizer tokenizer = new Tokenizer( this.BaseStream, Diagnostics.NullInteractionLog.Instance, bufferSize:16, XmlSerializationSettings.MaximumMessageSize ) )
            {
