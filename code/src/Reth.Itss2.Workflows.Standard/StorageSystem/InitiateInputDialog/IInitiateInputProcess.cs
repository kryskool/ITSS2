...
{
    public interface IInitiateInputProcess:IProcess
    {
        InitiateInputRequest Request{ get; }

        void SendResponse(  InitiateInputResponseDetails details,
                            IEnumerable<InitiateInputResponseArticle> articles  );

        Task SendResponseAsync( InitiateInputResponseDetails details,
                                IEnumerable<InitiateInputResponseArticle> articles  );

        void SendMessage(   InitiateInputMessageDetails details,
                            IEnumerable<InitiateInputMessageArticle> articles   );

        Task SendMessageAsync(  InitiateInputMessageDetails details,
                                IEnumerable<InitiateInputMessageArticle> articles   );
    }
}
