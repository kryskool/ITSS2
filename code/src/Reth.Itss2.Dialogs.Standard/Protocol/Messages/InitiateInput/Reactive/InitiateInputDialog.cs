...
namespace Reth.Itss2.Dialogs.Standard.Protocol.Messages.InitiateInput.Reactive
{
    public class InitiateInputDialog:Dialog, IInitiateInputDialog
    {
        public event EventHandler<MessageReceivedEventArgs<InitiateInputRequest>>? RequestReceived;
        ...
        public override void Connect( IMessageTransmitter messageTransmitter )
        {
            base.Connect(   messageTransmitter,
                            ( IMessage message ) =>
                            {
                                this.Dispatch<InitiateInputRequest>( message, this.OnRequestReceived );
                            }   );
        }
