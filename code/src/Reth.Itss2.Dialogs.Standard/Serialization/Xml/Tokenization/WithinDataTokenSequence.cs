...
{
    internal class WithinDataTokenSequence:TokenSequence
    {
        public WithinDataTokenSequence( IEnumerable<TokenStateTransition> transitions, long position )
        :
            base( transitions, position )
        {
        }

        public override TokenState State => TokenState.WithinData;

        public override TokenSequence Traverse( ref ReadOnlySequence<byte> buffer )
        {
            TokenSequence result = this;

            TokenPatternMatch match = buffer.FirstMatch( TokenPattern.EndOfData, this.Position );
