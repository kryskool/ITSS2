...
namespace Reth.Itss2.Dialogs.Standard.Serialization.Xml
{
    public class SerializationProvider:ISerializationProvider
    {
        ...
        public SerializationProvider( IInteractionLog interactionLog )
        :
            this( interactionLog, MessageTransmitter.DefaultMessageRoundTripTimeout )
        {
        }

        public SerializationProvider( IInteractionLog interactionLog, TimeSpan messageRoundTripTimeout )
        {
        ...
        private IMessageTransmitter CreateMessageTransmitter( Stream stream, IDataContractResolver dataContractResolver )
        {
            IMessageStreamReader messageStreamReader = new MessageStreamReader( stream, dataContractResolver, this.InteractionLog );
            IMessageStreamWriter messageStreamWriter = new MessageStreamWriter( stream, dataContractResolver, this.InteractionLog );
        ...
        {
            IMessageTransmitter messageTransmitter = this.CreateMessageTransmitter( stream, dialogProvider.DataContractResolver );
